...
class TwoAxisStage:
    ...
    def __init__(self, window, port, baud, startupfile):
        ...
        self.lbl_pos['font'] = font.Font(size=12)
        ...
        self.gcode_send.configure(width=self.buttonx, height=self.buttony)
        ...
        self.file_run.configure(width=self.buttonx, height=self.buttony)
        self.kill_btn = tk.Button(master=self.window, text='Kill', command=self.killSwitch)

        self.kill_btn.grid(row=5, column=5)
        self.kill_btn.configure(width=self.buttonx, height=self.buttony)
        ...
        self.home_btn = tk.Button(master=self.window, text='Go\nHome', command=lambda: self.sendCommand('G90 X0 Y0 F'
                                  + str(self.parameters['xMaxRate'][1])))
        ...
        self.start_from_death_btn['font'] = font.Font(size=7)
